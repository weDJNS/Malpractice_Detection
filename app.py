...
app.config['MYSQL_USER'] = '1mtSWSFK6h'
app.config['MYSQL_PORT'] = 3306
app.config['MYSQL_PASSWORD'] = 'XFFZ3g74c5'
app.config['MYSQL_DB'] = '1mtSWSFK6h'
app.config['MYSQL_CURSORCLASS'] = 'DictCursor'

app.config['MAIL_SERVER']='smtp.gmail.com'
app.config['MAIL_PORT'] = 465
app.config['MAIL_USERNAME'] = 'skdomains100@gmail.com'
app.config['MAIL_PASSWORD'] = 'iwantdomain'
...
def video_feed():
	if request.method == "POST":
		...
		cur.close()
		if(results > 0):
			print("recorded image of video");
			return "recorded image of video"
		else:
			print("error in video");
			return "error in video"
		print("Video_feed ran successfully");
		#return "returned from video_feed"
...
def create_test_lqa():
	...
	if request.method == 'POST' and form.validate_on_submit():
		noofquestion=request.form['questions']
		test_id = generate_slug(2)
		cur = mysql.connection.cursor()
		for i in range(int(noofquestion)):
			marks=float(request.form['mark'+str(i)])
			cur.execute('INSERT INTO longqa(test_id,qid,q,marks,uid) values(%s,%s,%s,%s,%s)', (test_id,str(i),request.form['question'+str(i)] ,marks , session['uid']))
		...
		proctor_type = 0
		cur.execute('INSERT INTO teachers (email, test_id, test_type, start, end, duration, show_ans, password, subject, topic, neg_marks, calc, proctoring_type, uid) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(dict(session)['email'], test_id, "subjective", start_date_time, end_date_time, duration, 0, password, subject, topic, 0, 0, proctor_type, session['uid']))
		mysql.connection.commit()
		cur.close()
	...
	if callresults['test_type'] == "objective":
		...
		if request.method == 'GET':
			...
			if session['uid']:
				...
				cur = mysql.connection.cursor()
				results1 = cur.execute('SELECT qid from longqa where test_id = %s',[testid])
				results1 = cur.fetchall()
				cur.close()
				insertStudentData = None
				print("yes")
				for sa in range(0,len(results1)):
					answerByStudent = request.form[results1[sa]['qid']]
					cur = mysql.connection.cursor()
					insertStudentData = cur.execute('INSERT INTO longtest(email,test_id,qid,ans,uid) values(%s,%s,%s,		%s,%s)', (session['email'], testid, results1[sa]['qid'], answerByStudent, session['uid']))
					mysql.connection.commit()

				if insertStudentData > 0:
					insertStudentTestInfoData = cur.execute('UPDATE studenttestinfo set completed = 1 where test_id =%s and email = %s and uid = %s', (testid, session['email'], session['uid']))
					mysql.connection.commit()
					cur.close()
